...
export const calculateMark = (node, multAnswers, form) => {
	...
	if (node.type === 2) {
		let acc = { min: 0, max: 0 };
		let resp = multAnswers[node.identifier]
			? getResponseByIdentifier(form, node.identifier)
			: {};
		...
		return nodes.map((node, index) => {
			...
			switch (node.type) {
				...
				case 1:
					...
					break;
				case 2:
					result = renderScoreMultipleNode(
						node,
						`${parentKey}-${index}`,
						debug,
						responses,
...
export const renderScoreMultipleNode = (data, key, debug, responses, form) => ({
	...
	title: (
		<span>
			...
			<Tag color={data.bool ? '#87d068' : '#f50'}>
				{data.title} {data.title && '|'}{' '}
				<Typography.Text strong>{data.identifier}</Typography.Text>
			</Tag>
			{data.feedback && renderFeedback(data.feedback, data.bool)}
		</span>
	),
...
function DecisionTreeF(props) {
	...
	const trace = (data, key, callback) => {
		...
		path.pop();
		let current = data;
		path.forEach((i) => (current = current[i].children));
	...
	const onRemoveConfirm = () => {
		Modal.confirm({
			...
			content: (
				<span>
					Do you want to delete this node? It will delete{' '}
					<span style={{ color: 'red' }}>its children</span> as well.
				</span>
			),
			onOk: onRemove,
		});
