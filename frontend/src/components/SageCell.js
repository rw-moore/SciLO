...
export default class SageCell extends React.Component {
	...
	componentDidMount() {
		let try_load = loadSageScript(
			this.props.src
				? this.props.src
				: 'http://127.0.0.1:8888/static/embedded_sagecell.js',
			'SageCellScript',
			() => {
				...
				const try_register = setInterval(() => {
					...
					if (time > 3000) {
						...
						console.error(
							'sagecell-react',
							'register onchange event timeout'
						);
					} else {
						time++;
					}
				...
				this.props.getCellInfoReference &&
					this.props.getCellInfoReference(cellInfo);
			}
		);
	...
	render() {
		if (!this.state.hidden) {
			...
			return (
				<div
					style={{ ...{ marginBottom: '10px' }, ...this.props.style }}
				>
					<div
						className={'SageCell'}
						id={this.state.id}
