...
function Cell(props) {
	...
	return (
		<Card
			...
			size={'small'}
		>
			<div className={'SageCellMultiple'} id={id}>
				<script
					type={props.language ? props.language : 'text/x-sage'}
					id={id}
				>
					{props.children}
				</script>
...
export default class SageCellMultiple extends React.Component {
	...
	componentDidMount() {
		if (!window.sagecell) {
			...
			script.src = this.props.src
				? this.props.src
				: 'http://live.vanillacraft.cn:8888/static/embedded_sagecell.js';
			//script.async = true;
	...
	render() {
		return (
			<div
				style={{ width: '75%', marginLeft: '12.5%', marginTop: '32px' }}
			>
				<Button
