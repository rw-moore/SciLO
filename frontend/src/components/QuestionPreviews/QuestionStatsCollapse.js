		...
		return Object.entries(inputs).map((item) => {
			for (var i = 0; i < this.props.question.responses.length; i++) {
				if (this.props.question.responses[i].identifier === item[0]) {
					if (
						this.props.question.responses[i].type.name !==
						'multiple'
					) {
						return (
							<div key={item[0]}>
							...
							<div key={item[0]}>
								{item[0]}:{' '}
								{
									<span>
										{this.renderMCText(
											item[1],
											this.props.question.responses[i]
												.answers
										)}
									</span>
								}
								...
								<DescriptionItem
									...
									content={
										<div style={{ marginLeft: 12 }}>
											...
											{this.props.question.grade_policy
												.penalty_per_try !==
												undefined && (
												<DescriptionItem
													title="Penalty Per Try"
													content={
														this.props.question
															.grade_policy
															.penalty_per_try
							...
							<Col xs={{ span: 24 }} md={{ span: 12 }}>
								<DescriptionItem
									...
									content={`${
										...
										this.props.question.tries
											? this.props.question.tries.length
											: this.props.question.grade_policy
													.max_tries
									}`}
								/>
								{this.props.question.tries &&
									this.props.question.tries.map(
										(attempt, index) => {
											if (attempt[0]) {
												return (
													<div key={index}>
															...
															attempt[2] && (
																<>
																	...
																	<Popover
																		content={
																			<pre>
																				{this.props.question.feedback.end.join(
																					'\n'
																				)}
																			</pre>
																		}
