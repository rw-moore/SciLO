...
export default class QuickLook extends React.Component {
	...
	componentDidUpdate(prevProps, prevState, snapshot) {
		if (this.state.current !== this.props.question.id) {
			this.setState({
				active: this.props.question.responses.map((r) =>
					r.id.toString()
				),
				current: this.props.question.id,
			});
	...
	static getTypeName(response) {
		...
		} else return response.type.name;
	}
			...
			<div>
				...
				<Row>
					<Col span={12}>
						<DescriptionItem
							title="Title"
							content={this.props.question.title}
						/>
					</Col>
					<Col span={12}>
						<DescriptionItem
							title="ID"
							content={this.props.question.id}
						/>
					</Col>
				</Row>
				<Row>
						...
						<DescriptionItem
							...
							content={
								this.props.courses.find(
									(c) => c.id === this.props.question.course
								)?.shortname ?? ''
							}
						/>
					</Col>
						...
						<DescriptionItem
							...
							content={
								<XmlRender
									...
									inline
									script={
										this.props?.question?.variables?.value
									}
								>
				...
				<Row>
					<Col span={24}>
						<DescriptionItem
							title="Tags"
							content={this.props.question.tags.map((tag) => (
				...
				<Row>
					<Col span={12}>
						<DescriptionItem
							title="Author"
							content={this.props.question.author}
						/>
					</Col>
					<Col span={12}>
						<DescriptionItem
							title="Quizzes"
							content={this.props.question.quizzes.toString()}
						/>
					</Col>
				</Row>
				<Row>
					<Col span={12}>
						<DescriptionItem
							title="Date Created"
							content={moment
								.utc(this.props.question.create_date)
								.format('lll')}
						/>
					</Col>
					<Col span={12}>
						<DescriptionItem
							title="Last Modified"
							content={moment
								.utc(this.props.question.last_modify_date)
								.format('lll')}
						/>
					</Col>
				</Row>
