...
export class Table {
	reference = {
		...
		M: {
			...
			method: (attrs) => ({
				type: Formula,
				props: { ...attrs, className: attrs.class },
			}),
		...
		m: {
			...
			method: (attrs) => ({
				type: Formula,
				props: { ...attrs, className: attrs.class, inline: true },
			}),
			example: '<m>\\frac{1}{\\sqrt{2}}\\cdot 2</m>',
		},
		v: {
		...
		dbox: {
			type: 'static',
			description:
				'Will render an embedded version of a dropdown box into the question text.',
			method: (attrs, data) => ({
				type: DBox,
				props: { ...attrs, className: attrs.class, data: data },
			}),
	...
	getTable = () =>
		Object.fromEntries(
			Object.entries(this.reference).map((tag) => [
				tag[0],
				tag[1].method !== undefined
					...
					: (attrs) => {
							...
							return {
								type: tag[0],
								props: { ...attrs, className: tag_class },
							};
					  },
			])
		);
