...
class UserInfoUpdateForm extends React.Component {
	...
	handleSubmit = (e) => {
		...
		this.props.form.validateFieldsAndScroll((err, values) => {
			if (!err) {
				...
				PatchUser(this.props.user.id, values, this.props.token).then(
					(data) => {
	...
	deleteAvatar = () => {
		...
		if (user.id) {
			DeleteAvatar(user.id, this.props.token).then((data) => {
				if (!data || data.status !== 200) {
					message.error(
						`Cannot delete avatar of ${this.props.name}, see browser console for more details.`
					);
					this.setState({
						loading: false,
					});
	...
	PutAvatar = () => {
		const user = this.props.user ? this.props.user : {};
		if (user.id) {
			PutAvatar(user.id, this.state.avatar, this.props.token).then(
				(data) => {
					if (!data || data.status !== 200) {
						message.error(
							`Cannot upload avatar of ${this.props.name}, see browser console for more details.`
						);
						this.setState({
							loading: false,
						});
	...
	sendEmailCaptcha = () => {
		...
		SendEmailCaptcha(this.props.token).then((data) => {
			if (!data || data.status !== 200) {
				if (data.status >= 400) {
					message.error(
						`Cannot send email verification captcha, see browser console for more details.`
					);
				} else {
				...
				<Form.Item
					...
					extra={
						this.props.user.email_active
							...
							: 'We must make sure that your email can be verified.'
					}
				>
					<Row gutter={8}>
						<Col span={12}>{this.props.user.email}</Col>
						{!this.props.user.email_active && (
							<div>
								<Col span={2} />
								<Col span={10}>
