...
class LoginForm extends React.Component {
			...
			.then((values) => {
				...
				UserLogin(values).then((data) => {
					if (!data || data.status !== 200) {
						message.error(
							`Login failed. ${
								data ? data.data.message : 'see browser console for more details'
							}`
						);
					} else {
	...
	onSignIn = (googleUser) => {
		...
		LoginWithGoogle({ id_token: googleUser }).then((data) => {
			if (!data || data.status !== 200) {
		...
		return (
			...
			>
				{(this.props.restrictions.length === 0 ||
					this.props.restrictions.includes('Google')) && (
					<>
						<Row justify="center">
							<Form.Item>
								<GoogleLogin
									onSuccess={this.onSignIn}
									onError={this.onGoogleFail}
									auto_select={true}
									text="continue_with"
									ux_mode="popup"
									hosted_domain="ualberta.ca"
								/>
							</Form.Item>
						</Row>
					</>
				...
				{(this.props.restrictions.length === 0 ||
					['Google', 'Username'].every((val) =>
						this.props.restrictions.includes(val)
					)) && <Divider style={{ marginTop: 0 }} />}
				{(this.props.restrictions.length === 0 ||
					this.props.restrictions.includes('Username')) && (
					<>
						<Form.Item
							name="username"
							rules={[
