	...
	export = () => {
		this.props.form
			...
			.then((fieldsValue) => {
				...
				sanitizeQuestions(
					this.props.order.map((id) => this.props.questions[id]),
					(questions) => {
			...
			.then((fieldsValue) => {
				...
				const values = {
					...
					questions: this.props.order.map((id) => ({
						id: id,
						mark: this.props.questions[id].mark,
					})),
				};
			...
			if (startTime) {
				if (!value.isSameOrAfter(startTime)) {
					return Promise.reject(
						new Error(
							'Oops, you have the end of the quiz earlier that the start of the quiz.'
						)
					);
				}
			}
		...
		if (value) {
			...
			if (endTime) {
				if (!value.isSameOrAfter(endTime)) {
					return Promise.reject(
						new Error(
							'Oops, you have the late submission time earlier than the end time.'
						)
					);
				}
			}
	...
	reviewOption = (title, name, description, disabled = []) => {
		...
		let tip = (title) => (
			<span style={{ marginLeft: 4 }}>
				<Tooltip title={title} trigger={'click'} placement="right">
			...
			<Space direction="vertical" size={[0, 1]}>
				...
				>
					<Checkbox disabled={disabled.findIndex((val) => val === 'correct') >= 0}>
						...
						{show_tip &&
							tip(
								'This covers the highlighting that denotes correct, partially correct or incorrect'
							)}
					</Checkbox>
				</Form.Item>
				<Form.Item
				...
				>
					<Checkbox disabled={disabled.findIndex((val) => val === 'feedback') >= 0}>
						...
						{show_tip &&
							tip(
								'The feedback given by the evaluation tree and any multiple choice fields they selected'
							)}
					</Checkbox>
				</Form.Item>
				<Form.Item
					...
					<div>
						...
						>
							<DatePicker
								...
								format={timeFormat}
								style={{ width: '100%' }}
								placeholder="Leave empty to NOT allow late submission"
							/>
						</Form.Item>
						<Form.Item
							noStyle={true}
							shouldUpdate={(prevValues, currentValues) =>
								prevValues.late_time !== currentValues.late_time
							}
						>
							{({ getFieldValue }) => (
								<Form.Item
									label={
										<Tooltip title="Penalty for submitting late">
											Deduction after deadline
										</Tooltip>
									}
					...
					<div>
						<Row>
							<Col span={12}>
								<Form.Item
									noStyle={true}
									shouldUpdate={(prevValues, currentValues) =>
										prevValues.options.max_attempts !==
										currentValues.options.max_attempts
									}
								>
									{({ getFieldValue }) => (
										<div>
											<Form.Item
												label={
													<Tooltip title="Max number of quiz attempts">
														Number of Attempts
													</Tooltip>
												}
								...
								<Form.Item
									label={
										<Tooltip title="Student can only submit one try per question per attempt">
											Only allow 1 try
										</Tooltip>
									}
								...
								<Form.Item
									label={
										<Tooltip title="Do not use a deduction per try within an attempt">
											No try deduction
										</Tooltip>
									}
						...
						<Row>
							<Col span={12}>
								<Form.Item
									label={
										<Tooltip title="Hide the quiz from the students.">
											Hidden
										</Tooltip>
									}
								...
								<Form.Item
									label={
										<Tooltip title="Allow users to take the quiz even if they are not in the course.">
											Outside course
										</Tooltip>
									}
							...
							<Col span={12}>
								<Form.Item
									label={
										<Tooltip title="Do not show the student the titles of the questions.">
		...
		return (
			<Form form={this.props.form} initialValues={defaults}>
				...
				<div className="steps-action">
					...
					{current === 2 && (
						<Button
							icon={<MoreOutlined />}
							onClick={() => {
								this.setState({ showQuestionBank: true });
							}}
							style={{ float: 'right' }}
						>
							Manage Questions
				...
				<QuestionBankModal
					...
					close={() => {
						this.setState({ showQuestionBank: false });
					}}
				/>
				<Drawer
					width={640}
				...
				<CreateQuestionModal
					...
					close={() => {
						this.setState({ showQuestionEditor: false });
						this.props.update(this.props.order);
					}}
				/>
			</Form>
		);
