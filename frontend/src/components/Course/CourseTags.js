...
export default class CourseTags extends React.Component {
	...
	fetch = () => {
		...
		GetCourseById(this.props.course, this.props.token).then((data) => {
		...
		if (this.state.course) {
			return (
				<Link to={`/Course/${this.state.course.id}`}>
					<Tag
						style={{ float: 'right' }}
						color={RandomColorBySeed(this.state.course.id).bg}
					>
						<span
							style={{
								color: RandomColorBySeed(this.state.course.id)
									.fg,
							}}
						>
							{this.state.course.shortname}
						</span>
					</Tag>
				</Link>
			);
