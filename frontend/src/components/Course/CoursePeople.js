					...
					rules={[
						...
						() => ({
							async validator(_, value) {
								GetUserByUsername(value, props.token).then(
									(data) => {
										console.log('test', data);
										if (!data || data.status !== 200) {
											return Promise.reject(
												new Error(
													'User not found or network issue.'
												)
											);
										} else {
											console.log(data.data);
...
export default class CoursePeople extends React.Component {
		...
		this.formRef1.current
			...
			.then((values) => {
				...
				AddUserByUsername(
					...
					if (!data || data.status !== 200) {
						message.error(
							'Cannot add people, see browser console for more details.'
						);
						this.setState({
							loading: false,
						});
	...
	handleCreateGroup = () => {
		this.formRef2.current
			...
			.then((values) => {
				console.log('Received values of form: ', values);
				PostGroup(values, this.props.course, this.props.token).then(
					(data) => {
						if (!data || data.status !== 200) {
							message.error(
								'Cannot create group, see browser console for more details.'
							);
							this.setState({
								loading: false,
							});
	...
	remove = (username, group) => {
		...
		RemoveUserByUsername(
			...
			if (!data || data.status !== 200) {
				message.error(
					'Cannot remove people, see browser console for more details.'
				);
				this.setState({
					loading: false,
				});
