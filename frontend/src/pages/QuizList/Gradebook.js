...
export default class Gradebook extends React.Component {
	...
	fetch = (params = {}) => {
		GetQuizGradebookById(this.props.id, this.props.token, params).then(
			(data) => {
	...
	getSummaryAttempts = () => {
		...
		Object.entries(this.state.attempts).forEach((entry) => {
			...
			for (let i = 0; i < this.state.max_num; i++) {
				...
				data[2].questions[i].s2 =
					data[2].questions[i].s2 + qgrade * qgrade;
				data[0].questions[i].grade = Math.min(
					data[0].questions[i].grade,
					qgrade
				);
				data[1].questions[i].grade = Math.max(
					data[1].questions[i].grade,
					qgrade
				);
			}
		});
		...
		for (let i = 0; i < this.state.max_num; i++) {
			...
			data[2].questions[i].grade = Math.sqrt(
				(p1 - p2 * p2) * (n / (n - 1))
			);
		}
		return data;
