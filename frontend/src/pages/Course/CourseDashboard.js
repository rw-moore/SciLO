...
class Course extends React.Component {
	...
	handleCreate = () => {
		this.formRef1.current
			...
			.then((values) => {
				...
				PostCourse(values, this.props.token).then((data) => {
					if (!data || data.status !== 200) {
						message.error(
							'Cannot create course, see browser console for more details.'
						);
						this.setState({
							fetching: false,
						});
	...
	handleEnroll = () => {
		this.formRef2.current
			...
			.then((values) => {
				console.log('Received values of form: ', values);
				EnrollCourse(values, this.props.token).then((data) => {
					if (!data || data.status !== 200) {
						message.error(
							data.data.message ||
								'Cannot enroll in course, see browser console for more details.'
						);
						this.setState({
							fetching: false,
						});
