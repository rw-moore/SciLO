...
class QuizTestCase(TestCase):
    ...
    def test_serializer(self):
        ...
        QuizQuestion.objects.create(quiz=self.quizzes[2],
                                    question=self.questions[2])
        print(QuizQuestion.objects.all()) 
        self.test_save()

        serialize = QuizSerializer(self.quizzes[2])
        import json
        data = json.loads(json.dumps(serialize.data))
        # print(data)
    # def test_deserializer(self):
    #     QuizCategory.objects.create(title='c1')
    #     User.objects.create_user(
    #         username="haotian",
    #         password="dsahsdhsakda")
    #     data = {
