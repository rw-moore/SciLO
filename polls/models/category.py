    ...
    def create_from_json(cls,json_dict):
        ...
        if category_dict:
            ...
            instance.questions.set(
                [ Question.create_from_json(question_dict) \
...
class QuizCategory(Category):
    ...
    def to_json(self, exclude=[]):
        ...
        d = {
            ...
            'last_modify_date': str(self.last_modify_date),
            'author': self.author.id if self.author else None,
            'childrens': [child.id for child in self.children.all()],
            'quizzes': [q.id for q in self.quizzes.all()]
        }
        for key in exclude:
            d.pop(key)
        return d
...
class QuestionCategory(Category):
    ...
    def to_json(self, exclude=[]):
        ...
        d = {
            ...
            'author': self.author.id if self.author else None,
            'childrens': [child.id for child in self.children.all()],
            'questions': [q.id for q in self.questions.all()]
        }
        for key in exclude:
            d.pop(key)
        return d